...
For a local release-format preview:

```sh
npm run gateway:prepare-deploy
```

The release contains only compiled Gateway files, pinned production manifests, source/workflow provenance, and checksums. The VPS deploys it into a versioned directory, atomically promotes it, requires a ready health response, and restores the previous release if startup fails. Runtime configuration stays in the VPS `/etc` file and never enters GitHub or the release artifact. See [the Gateway deployment guide](docs/gateway-vps-deploy.md) for the security and rollback model.
